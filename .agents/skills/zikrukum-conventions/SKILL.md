...
- Offline-first: all content bundled locally (`src/dataset/azkar.json` + fonts + `assets/audio/` clips). No new network dependencies for core content.
...
- Data flow: `azkar.json` -> `src/mappers/azkarMapper.ts` -> typed `AzkarCategory[]` -> `CategoryScreen` dispatches `setPhases` -> `PhraseCard` FlatList pager.
